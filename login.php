...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Hardcoded credentials for Student (Ammar)
    $studentUsername = 'Ammar';
    $studentPassword = '123';
    $studentRole = 'pelajar';
    ...
    // Check if the user is trying to log in as Student (Ammar)
    else if ($user_type == 'pelajar') {
        // Check if the credentials match the hardcoded Student (Ammar)
        if ($username == $studentUsername && $password == $studentPassword) {
            // Set session variables for student
            $_SESSION['user_id'] = 2;  // Hardcoded Student ID
            $_SESSION['username'] = $studentUsername;
            $_SESSION['role'] = $studentRole;
            ...
            $message = "Tidak sah nama pengguna atau kata laluan.";  // Invalid username or password
        }
    }
}
?>
